...
$(function () {
    /*
    commonActionsPage: '/api/business/commAction/page',//通用按钮
    */
    ...
    $('#commActionDefineSubmitButton').bind("click", function () {
        $("#processPage").val(1);
    });
    $('#commActionDefineSubmitButton').trigger("click");
});

function queryProcessList() {//查询业务定义列表
    var _url = window.top.contextPath + window.portPath.commonActionsPage;
    ...
    window.common.networkConnect(_url, "POST", window.conType.conTyJson, queryForm, function (res) {
        if (res.exchangeStatus == 1) {
            window.common.drawTemplateHtml("commActionsTemplate", "#commActionsTb", {listData: res.data});
            var opts = {
...
var commActionDefineLayerDialog = null;

function showEditDialog(flag) {//打开编辑弹窗
    var _title = '';
    if (flag == 'add') {
        _title = '新增';
        $(':input', '#commActionDefineForm').not(':button,:submit,:reset').val('');
    } else if (flag == 'update') {
        _title = '修改';
        ...
        getCommAction(function (listData) {
            for (var key in listData) {
                $("#commActionDefineForm input[name='" + key + "']").val(listData[key]);
                $("#commActionDefineForm select[name='" + key + "']").val(listData[key]);
            }
        });
    }
    commActionDefineLayerDialog = layer.open({
        title: _title,
        type: 1,
        area: [550, 330],
        content: $("#commActionDefineFormDialog"),
        // validate:$("#commActionDefineForm").validate().resetForm(),
    });
...
function closeEditDialog() {//关闭编辑弹窗
    if (commActionDefineLayerDialog) {
        // $("#commActionDefineForm").validate().resetForm();
        layer.close(commActionDefineLayerDialog);
    } else {
        layer.closeAll();
    }
}
...
function catalogAction() {
    var _url = window.top.contextPath + window.portPath.findChildCataLogList + "?pcodes=BUSINESS_ACTION";
    window.common.networkConnect(_url, "GET", window.conType.conTyForm, null, function (res) {
        if (res.exchangeStatus == 1) {
            var initSelectData = {
                contextPath: window.top.contextPath,
                CatalogActions: res.data,
            };
            window.common.drawTemplateHtml("catalogActionList", ".catalogActionTb", initSelectData);
            window.common.drawTemplateHtml("catalogActionListTt", ".catalogActionTt", initSelectData);
        }
    });
}
...
function submitFormData() {//提交表单数据
    $("#commActionDefineForm").validate({
        ...
        submitHandler: function (form) {
            var paramsObject = $("#commActionDefineForm").serializeObject();
            paramsObject['catalogName'] = $("#catalogName").val();
            var _url = window.top.contextPath + window.portPath.commonActionsSave;
...
function removeCommActionDefineByCode() {//根据业务id移除数据
    var code = $("#selectedId").val();
    if (!code || code == "") {
    ...
    layer.confirm("确认删除", {
        ...
        var code = $("#selectedId").val();
        var _url = window.top.contextPath + window.portPath.commonActionsDelete + "?id=" + code;
...
function getCommAction(callback) {//根据流程id获取信息
    var id = $("#selectedId").val();
    var _url = window.top.contextPath + window.portPath.commonActionsFindOne + "?id=" + id;
    window.common.networkConnect(_url, "GET", window.conType.conTyJson, null, function (res) {
        if (res.exchangeStatus == 1) {
