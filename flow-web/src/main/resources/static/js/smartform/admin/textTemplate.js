...
$(function(){
	...
	window.common.getNiceScroll("wrapper");
	$('#queryTextTemplateForm').on('submit',function(){
...
function getTextTemplateByBusinessId(){//通过业务ID查询模板列表
	...
	window.common.networkConnect(_url, "POST", window.conType.conTyJson, queryForm, function(res) {
        if (res.exchangeStatus == 1) {
        	window.common.drawTemplateHtml("textTemplate", "#textTemplateTb", {textTemplateData: res.data});
        	var opts = {
